...
class Model(nn.Module):
    def __init__(self, config, load_emb=True, path=None):
        ...
        self.hidden_size = config.hidden_size
        self.draft_vocab_size =  config.vocab_size//4
        ...
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size//4, bias=False)
    ...
    def forward_train(
        ...
        vlosses = []
        acces = []
        '''for idx in range(random.randint(0,self.length-1)):
            with torch.no_grad():
                inputs_embeds = self.embed_tokens(input_ids)
                inputs_embeds = inputs_embeds.to(hidden_states.dtype)
                layer_outputs, cache_hidden = self.midlayer(
        ...
        for idx in range(self.length):
            ...
            with torch.no_grad():
                ...
                acces.append(((logits_new.argmax(-1) == target_max_token) * position_mask.squeeze(-1)).sum().item() / (
            ...
            if not last:
                '''with torch.no_grad():
                    hidden_states = torch.roll(hidden_states,1,1)
                    hidden_states.required_grad = False
    ...
    def scandata(self, datapath, tokenizerpath):
        ...
        if not os.path.exists("cache.pt"):
            ...
            dataset = Dataset0.from_file(f'/mnt/efs/people/yawenwuu/projects/Qwen2.5-Coder/finetuning/sft/dataset/code_edit_efs/context/hunks_v2/merged_output_part_0_39_mixed_pc_non_pc_dedup_threshold_09_chat_tokenized_v2_user_masked_with_codeediteval_bugbash/train_packed_max_len_8192/data-00001-of-00015.arrow')
            #dataset  = concatenate_datasets([Dataset0.from_file(f'/mnt/efs/people/yawenwuu/projects/Qwen2.5-Coder/finetuning/sft/dataset/code_edit_efs/context/hunks_v2/merged_output_part_0_39_mixed_pc_non_pc_dedup_threshold_09_chat_tokenized_v2_user_masked_with_codeediteval_bugbash/train_packed_max_len_8192/data-000{"{:02d}".format(i)}-of-00015.arrow') for i in range(1,1)])
