    ...
    with torch.no_grad():
        for i in range(len(data["input_ids"])):
            if len(data["input_ids"][i])<4096:
                print('sending tensor', torch.tensor([data["input_ids"][i][:4096]]).shape)
                ...
                td={"input_ids":input_ids[:4096].detach().cpu().numpy(),"hidden_state":None,'label':np.array([j for j in data['label'][i][:4096]]) }
                return td
