...
def build_dataset_rank(
    ...
    ds = concatenate_datasets([Dataset.from_file(f'/mnt/efs/people/pramudi/Eagle/data-base/data-000{"{:02d}".format(i)}-of-00096.arrow') for i in range(0,96)])
    ...
    return ds1
bigtokenizer = AutoTokenizer.from_pretrained(bigname,use_fast=False) #features: ['test_input_ids', 'test_output_ids', 'input_ids', 'label', 'metadata', 'category', 'thinking_or_reasoning', 'new_category_recommendation'],
...
def ge(data):
    with torch.no_grad():
        for i in range(len(data["input_ids"])):
            if len(data["input_ids"][i])<4096:
                input_ids=torch.tensor([data["input_ids"][i][:4096]])
                ...
                td={"input_ids":input_ids[:4096].detach().cpu().numpy(),"hidden_state":hidden_state_big.detach().cpu()[0][:4096].numpy(),'label':np.array([j for j in data['label'][i][:4096]]) }
                return td
